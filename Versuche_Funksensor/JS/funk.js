...
var data = Buffer.from("Hallo wie gehts?" + 1234);
  

    process.stdout.write("\nSending....");
    roundtrip=process.hrtime();
    if(do_sync) {
if(radio.write(data)) process.stdout.write("[Sync] Sended " + data);
       else process.stdout.write("[Sync] Failed to send ");
    } else {
      radio.write(data,function(success){
	if(success) process.stdout.write("[Async] Sended " + data);
        else process.stdout.write("[Async] Failed to send ");
      });
    }
  };
