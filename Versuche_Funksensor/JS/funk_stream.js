...
const green = new Gpio(26, {mode: Gpio.OUTPUT});
const blue = new Gpio(6, {mode: Gpio.OUTPUT});


function glowLED(redColour, greenColour, blueColour) {
	setInterval(() => {
 	 	green.pwmWrite(greenColour);
	 	red.pwmWrite(redColour);
	 	blue.pwmWrite(blueColour);
	}, 20);
}
...
radio.changeWritePipe(true, 2048);
	...
	radio.stream(data, (s, tx_ok, tx_bytes, tx_req, frame_size, aborted) => {
		if(s) { 
			console.log("Stream ok");
			glowLED(255,0,255);
		} else {
			console.log("error " + data); // gibt im Moment immer einen Error, egal, was man eingibt -> vielleicht weil reciever fehlt?
			glowLED(0,255,255);
		} 
